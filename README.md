...
# Generate 1 transaction and print it (no Kafka publishing)
python src/transaction_generator.py

# Generate and print transactions without publishing to Kafka
python src/transaction_generator.py --transactions 20

# 10 transactions/sec for 60 seconds, publishing to Kafka
python src/transaction_generator.py --rate 10 --duration 60 --publish

# Exactly 500 transactions published to Kafka, then exit
...
python src/transaction_generator.py --load-ref-data-from-db --publish

# Write generated transactions back to Astra DB as well
python src/transaction_generator.py --transactions 1000 --write-transactions-to-db --publish
